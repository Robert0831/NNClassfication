...
for iv in range(0,len(testset),5):
        ...
        Y_pred3 = model3.forward(imgv)
        out3=np.argmax(Y_pred3,1)
        for j in range(out3.shape[0]):
            if out1[j]==labelv[j]:
                score3+=1

print(f' two-layer top-1 accuracy:{round(score1/totalt,5)} ;LeNet5 top-1 accuracy:{round(score2/totalt,5)} ;NLeNet5 top-1 accuracy:{round(score3/totalt,5)}') 
